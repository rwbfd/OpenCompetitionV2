...
class EntityDenseLayer(nn.Module):
    def __init__(self, data_pd, opt: TabDataOpt, centroid = None,  mlp = None):
        ...
        if self.opt.dis_vars_vic is not None and centroid is not None:
            self.entity_v = nn.ModuleDict()
            for name in self.opt.dis_vars_vic:
                self.entity_v[name] = EntityEmbeddingVLayer(len(data_pd[name].unique()), self.opt.num_dim, centroid[name], name)
